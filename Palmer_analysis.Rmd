...
---
title: "Palmer Penguins Reproducible Analysis"
output:
...
```{r  Package installation, echo=FALSE, warning=FALSE}
if(!require("ggplot2")) 
...
```{r Package Loading, warning=TRUE}
library(ggplot2)
library(dplyr)
library(janitor)
...
Code to plot the bad graph
```{r  Bad Graph, warning=FALSE, echo=FALSE}
colors<- c("azure3", "azure4", "black")
ggplot(data = penguins)+
       aes(x= body_mass_g,
           y = bill_length_mm, color = species)+
  scale_colour_manual(values = colors)+
       geom_point(size=7)+
  geom_smooth(method = "lm", SE = FALSE, linetype = "solid", colour = "black")+
  labs(title = "A graph to show the relationship between Body mass and Bill Length in penguins", x = "Body Mass (g)",y = "Bill Length (mm)")+
  xlim(2000,6000)+
  ylim(10,100)
echo=FALSE
...
The grey background and white grid lines make following the relative position of the data on both axes difficult for the reader, and thus make the underlying data more difficult to understand in the context of the graph. The second design choice which may mislead a reader of this graph is the inappropriate scales on both the  X and Y axes. Both axes do not start in relevant positions to allow the spread of the data that they scale to be shown on the graph as efficiently as possible, meaning that the data is confined to a lower proportion of the available graph space making it more difficult to ascertain the trends shown in the data. Furthermore the X axis scale design actually cuts of a small number of data points for penguins with both high body mass and culmen length, thus this scale choice means that the data points shown on the graph do not accurately depict the data used to plot the line of best fit. A further design choice which damages the reader's understanding are the colours chosen to represent the 3 different penguin species, which are all similar to each other and to the grey background of the graph. This issue is furthered by using a very large point size argument which means that the darker coloured points often completely block out other underlying data meaning that not all data for all species is visible to the reader. Finally the application of a black line of best fit matching the colour of the Gentoo penguin data, which represents the line of best fit across all 3 species, may mislead the reader into believing this line applies to the Gentoo data further reducing its efficacy in portraying the trends of the underyling data to the reader.
